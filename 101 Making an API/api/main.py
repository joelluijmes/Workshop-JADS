...
def train_model_blocking(data):
    ...
    sarimax = SARIMAX(y_train, order=(0, 0, 0), seasonal_order=(1, 1, 1, 30),
    ...
    sarimax = SARIMAX(y, order=(0, 0, 0), seasonal_order=(1, 1, 1, 30),
                      enforce_invertibility=False, enforce_stationarity=False)
    updated_model = sarimax.fit()
...
async def post_train_model(request):
    ...
    with ProcessPoolExecutor() as pool:
        updated_model, mae = await asyncio.get_event_loop().run_in_executor(pool, partial(train_model_blocking, data))

    global model_mae, model
    if model_mae is None or mae < model_mae:
        model = updated_model
        model_mae = mae

        return web.Response(text='Model updated with MAE of {:.2f}'.format(mae))

    return web.Response(text='Model is not updated, previous MAE {:.2f} was <= than current {:.2f}'.format(model_mae, mae))
